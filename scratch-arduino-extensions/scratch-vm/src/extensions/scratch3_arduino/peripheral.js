...
const BLEDataStoppedError =
  "arduino nano 33 BLE Sense extension stopped receiving data";
...
class ArduinoPeripheral {
  ...
  _onConnect() {
    this._ble.startNotifications(
      BLEUUID.SERVICE,
      BLECharacteristic.SENSORS_DATA,
      this._onMessage,
    );
  ...
  _onMessage(base64) {
    ...
    if (window.arduinoBLEDebug && window.arduinoBLEDebug.message) {
      log.info(
        `Arduino BLE message: state=${msgState} sensors=${this._sensors} pins=${this._pins}`,
      );
    }
  ...
  setDigitalMode(pinNumber, mode) {
    const pinAction = [BLE_PIN_ACTIONS.PINMODE, pinNumber, mode, 0xff];

    return this._send(BLECommand.PIN_ACTIONS, pinAction);
  }

  setDigitalValue(pinNumber, value) {
    const pinAction = [BLE_PIN_ACTIONS.DIGITALWRITE, pinNumber, value, 0xff];

    return this._send(BLECommand.PIN_ACTIONS, pinAction);
  }

  setAnalogValue(pinNumber, value) {
    const pinAction = [BLE_PIN_ACTIONS.ANALOGWRITE, pinNumber, value, 0xff];
  ...
  stopServoMotor(pinNumber) {
    const pinAction = [BLE_PIN_ACTIONS.SERVOSTOP, pinNumber, 0xff];

    return this._send(BLECommand.PIN_ACTIONS, pinAction);
  }

  moveForward(steps) {
    const robotAction = [ROBOT_ACTIONS.MOVE_FORWARD, steps];
    return this._send(BLECommand.ROBOT_ACTIONS, robotAction);
  }

  moveBackward(steps) {
    const robotAction = [ROBOT_ACTIONS.MOVE_BACKWARD, steps];
    return this._send(BLECommand.ROBOT_ACTIONS, robotAction);
  }

  moveForwardTime(milliseconds) {
    let [arg0, arg1] = convertUInt16ToBytes(milliseconds);
    const robotAction = [ROBOT_ACTIONS.MOVE_FORWARD_TIME, arg0, arg1];
    return this._send(BLECommand.ROBOT_ACTIONS, robotAction);
  }

  moveBackwardTime(milliseconds) {
    let [arg0, arg1] = convertUInt16ToBytes(milliseconds);
    const robotAction = [ROBOT_ACTIONS.MOVE_BACKWARD_TIME, arg0, arg1];
    return this._send(BLECommand.ROBOT_ACTIONS, robotAction);
  }

  turnLeft(milliseconds) {
    let [arg0, arg1] = convertUInt16ToBytes(milliseconds);
    const robotAction = [ROBOT_ACTIONS.TURN_LEFT, arg0, arg1];
    return this._send(BLECommand.ROBOT_ACTIONS, robotAction);
  }

  turnRight(milliseconds) {
    let [arg0, arg1] = convertUInt16ToBytes(milliseconds);
    const robotAction = [ROBOT_ACTIONS.TURN_RIGTH, arg0, arg1];
    return this._send(BLECommand.ROBOT_ACTIONS, robotAction);
  }

  setSpeed(speed) {
    const robotAction = [ROBOT_ACTIONS.SET_SPEED, speed];
    return this._send(BLECommand.ROBOT_ACTIONS, robotAction);
  }
